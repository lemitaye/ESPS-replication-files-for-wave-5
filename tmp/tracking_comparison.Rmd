...
track_ea_dropped <- read_dta("track_ea_dropped.dta") %>% 
  mutate(
...
# Tracking table for DNA households
track_dna_hh <- read_dta("track_dna_hh.dta") %>% 
  clean_names()

track_dna_ea <- read_dta("track_dna_ea.dta") %>% 
  clean_names()


track_dna_hh_tbl <- summ_region(track_dna_hh) %>% 
  form_tab()

track_dna_ea_tbl <- bind_rows(
    summ_region(track_dna_ea) %>% 
      count(region, merge) %>% 
      complete(region, merge, fill = list(n = 0)),
  ...
  mutate(
      ...
      new_added = 0,  # need to find a way to repair this
      nobs_ess4 = matched + dropped,
      nobs_ess5 = matched + new_added,
      attr_rate = paste0(round((dropped / nobs_ess4) * 100, 1), "%")
      ) %>% 
...
kbl(
  track_dna_hh_tbl,
  caption = "Number of rural households with Maize-DNA data in ESPS4 and ESPS5 by region",
  booktabs = TRUE,
  align = c("l", "c", "c", "c", "c", "c", "c"),
  col.names = c("Region", "Matched", "Dropped in ESPS5 (only in ESPS4)", "Newly added in ESPS5", "No. of hhs in ESPS4 (col. 1 + col. 2)", "No. of hhs in ESPS5 (col. 1 + col. 3)", "Attrition Rate (col. 2 / col. 4)")
) %>% 
...
kbl(
  track_dna_ea_tbl,
  caption = "Number of rural EAs with Maize-DNA data in ESPS4 and ESPS5 by region",
  booktabs = TRUE,
  align = c("l", "c", "c", "c", "c", "c", "c"),
  col.names = c("Region", "Matched", "Dropped in ESPS5 (only in ESPS4)", "Newly added in ESPS5", "No. of EAs in ESPS4 (col. 1 + col. 2)", "No. of EAs in ESPS5 (col. 1 + col. 3)", "Attrition Rate (col. 2 / col. 4)")
...
track_dna_cov <- read_dta("track_dna_cov.dta") %>% 
  clean_names() %>% 
  summ_region()

labels <- var_label(track_dna_cov) %>% 
  .[-c(1:3, 16)] %>% 
  as_tibble() %>% 
  pivot_longer(
    cols = everything(), 
...
dna_ttest <- track_dna_cov %>% 
  # select(-household_id, -ea_id, -region) %>% 
  pivot_longer(cols = hhd_flab:totconswin,
               names_to = "covariate",
               values_to = "value") %>% 
  group_by(covariate) %>%
  summarize(
    t_test = list(broom::tidy(t.test(value ~ merge))),
    n = n(),
    .groups = "drop"
  ) %>% 
  unnest(t_test)
...
dna_ttest_tab <- dna_ttest %>% 
  mutate(
    difference = case_when(
      p.value <= .01 ~ paste0(round(estimate, 3), "***"),
      (p.value > .01 & p.value <= .05) ~ paste0(round(estimate, 3), "**"),
      (p.value > .05 & p.value <= .1) ~ paste0(round(estimate, 3), "*"),
      p.value > .1 ~ as.character(round(estimate, 3))
  ),
  matched = round(estimate2, 3),
  dropped = round(estimate1, 3)
  ) %>% 
  left_join(labels, by = "covariate") %>% 
  select(label, dropped, matched, difference) 
...
kbl(
  dna_ttest_tab,
  caption = "Comparison of mean of covariates for households with DNA data (results of a t-test)",
  booktabs = TRUE,
  align = c("l", "c", "c", "c"),
  col.names = c("Variable", "Dropped in ESPS5 (only in ESPS4)", "Matched", "Difference")
) %>% 
  column_spec(1, width = "18em", latex_valign = "b") %>% 
  column_spec(2:4, width = "8em", latex_valign = "b") %>% 
